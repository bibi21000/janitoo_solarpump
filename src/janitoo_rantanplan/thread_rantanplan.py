...
def make_thread(options, force=False):
    if get_option_autostart(options, OID) or force:
        return RantanplanThread(options)
    else:
        return None

class RantanplanThread(JNTBusThread):
    """The basic thread
    ...
    def init_bus(self):
        ...
        self.section = OID
        self.bus = RantanplanBus(options=self.options, oid=self.section, product_name="Raspberry rantanplan controller")
